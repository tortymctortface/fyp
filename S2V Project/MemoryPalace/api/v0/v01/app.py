...
def find_all_begining_with (doc):
    ...
    with open("api/v0/v01/vocab/vocab.txt","r", encoding="utf-8") as f:
        ...
        for file in text_files:
            with open(("api/v0/v01/textfiles/"+ file) , "w+", encoding="utf-8") as f:
...
def only_verbs_please (doc):
    ...
    for file in find_all_begining_with(doc):
        with open(("api/v0/v01/textfiles/"+ file),"r", encoding="utf-8") as f:
            TEXT = f.read()
            docu = nlp(TEXT)
        with open(("api/v0/v01/textfiles/"+ file),"w", encoding="utf-8") as f:
            for token in docu:
    ...
    for lst in div_in_list:
        ...
        for in_token in doc:
            ...
            if not in_token.text.isalpha():
                ...
                x = 1
                with open(("api/v0/v01/textfiles/text" + (str(in_token.text[0])).lower() + ".txt") , "r", encoding="utf-8") as f:
