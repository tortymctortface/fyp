...
with open("api/v0/v02/input_list/input_list.txt","r", encoding="utf-8") as f:
    TEXT = f.read()
doc = nlp(TEXT)
...
def create_first_letter_files(doc):
    ...
    with open("api/v0/v02/vocab/vocab.txt","r", encoding="utf-8") as f:
        ...
        for file in text_files:
            with open(("api/v0/v02/textfiles/"+ file) , "w+", encoding="utf-8") as f:
...
def create_output_list(doc, in_theme):
    ...
    with open("api/v0/v02/output/scores.csv", mode='w+') as f:
        f.close()
    ...
    for lst in div_in_list:
        ...
        for in_token in doc:
            ...
            if not in_token.text.isalpha():
                ...
                x = 1
                with open(("api/v0/v02/textfiles/text" + (str(in_token.text[0])).lower() + ".txt") , "r", encoding="utf-8") as f:
                    ...
                    for token in avail_vocab:
                        ...
                        if token.i == (len(avail_vocab)-1): 
                            ...
                            word_to_append = token
                    f.close()
    with open("api/v0/v02/output/output.txt", mode='w+') as f:
        f.write("Output List:")
        f.write("\n")
        f.write(str(result))
        f.close()
...
def create_output_csv(original,w1,w2,w3,theme):
    ...
    x = 1
    with open("api/v0/v02/output/scores.csv", mode='a') as csv_file:
...
def create_verbs_only_file ():
    ...
    with open("api/v0/v02/vocab/vocab.txt","r", encoding="utf-8") as f:
        vocab = nlp(f.read())
        with open(("api/v0/v02/textfiles/verbs") , "w+", encoding="utf-8") as f:
            for token in vocab:
                if token.pos_ == "VERB":
                    f.write(token.lemma_ + " ")
            f.close()
    ...
    for word in midlist:
        ...
        if x == (len(midlist)):
            ...
            if word.pos_ == "DET" or word.pos_ == "CCONJ" or word.pos_ == "ADP" or next_word.pos_ == "DET" or next_word.pos_ == "CCONJ" or next_word.pos_ == "ADP" :
                ...
                outlist.append(word.text)
                with open(("api/v0/v02/textfiles/verbs") , "r", encoding="utf-8") as f:
                    vocab = nlp(f.read())
                    for token in vocab:
                        #try catch to handle the possibility that a comparison between to words (the theme word and the current word from the vocab) has never occured before
                        try:
                        ...
                        if token.i == (len(vocab)-1):
                            ...
                            verb_to_append = token
                    f.close()
    with open("api/v0/v02/output/output.txt", mode='a') as f:
        f.write("\n")
        f.write("\n")
