...
with open("api/v03/input_list/input_list.txt","r", encoding="utf-8") as f:
    TEXT = f.read()
doc = nlp(TEXT)
...
def create_first_letter_files(doc):
    ...
    with open("api/v03/vocab/nouns.txt","r", encoding="utf-8") as f:
        ...
        for file in text_files:
            with open(("api/v03/textfiles/"+ file) , "w+", encoding="utf-8") as f:
...
def create_output_list(doc, in_theme,pw,slw):
    ...
    with open("api/v03/output/scores.csv", mode='w+') as f:
        f.close()
    ...
    for in_token in doc:
        ...
        if not in_token.text.isalpha():
            ...
            x = 1
            with open(("api/v03/textfiles/text" + (str(in_token.text[0])).lower() + ".txt") , "r", encoding="utf-8") as f:
                ...
                for token in avail_vocab:
                    ...
                    if token.i == (len(avail_vocab)-1): 
                        ...
                        word_to_append = token
                f.close()
    with open("api/v03/output/output.txt", mode='w+') as f:
        f.write("Output List:")
        f.write("\n")
        f.write(str(result))
        f.close()
...
def create_output_csv(original,w1,w2,w3,theme,pw,slw):
    ...
    x = 1
    with open("api/v03/output/scores.csv", mode='a') as csv_file:
...
def create_verbs_only_file ():
    ...
    with open("api/v03/vocab/vocab.txt","r", encoding="utf-8") as f:
        vocab = nlp(f.read())
        with open(("api/v03/textfiles/verbs") , "w+", encoding="utf-8") as f:
            for token in vocab:
                if token.pos_ == "VERB":
                    f.write(token.lemma_ + " ")
            f.close()
    ...
    for word in midlist:
        ...
        if x == (len(midlist)):
            ...
            if word.pos_ == "DET" or word.pos_ == "CCONJ" or word.pos_ == "ADP" or next_word.pos_ == "DET" or next_word.pos_ == "CCONJ" or next_word.pos_ == "ADP" :
                ...
                outlist.append(word.text)
                with open(("api/v03/textfiles/verbs") , "r", encoding="utf-8") as f:
                    vocab = nlp(f.read())
                    for token in vocab:
                        #try catch to handle the possibility that a comparison between to words (the theme word and the current word from the vocab) has never occured before
                        try:
                        ...
                        if token.i == (len(vocab)-1):
                            ...
                            verb_to_append = token
                    f.close()
    with open("api/v03/output/output.txt", mode='a') as f:
        f.write("\n")
        f.write("\n")
