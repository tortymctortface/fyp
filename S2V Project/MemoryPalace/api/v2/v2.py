...
def create_first_letter_files(doc):
    ...
    with open("api/v2/vocab/nouns.txt","r", encoding="utf-8") as f:
        ...
        for file in text_files:
            with open(("api/v2/textfiles/"+ file) , "w+", encoding="utf-8") as f:
...
def create_output_list_v2(doc, in_theme,pw,slw,flw):
...
def create_verbs_only_file ():
    ...
    with open("api/v2/vocab/vocab.txt","r", encoding="utf-8") as f:
        vocab = nlp(f.read())
        with open(("api/v2/textfiles/verbs") , "w+", encoding="utf-8") as f:
            for token in vocab:
                if token.pos_ == "VERB":
                    f.write(token.lemma_ + " ")
            f.close()
    ...
    for word in midlist:
        ...
        if x == (len(midlist)):
            ...
            if word.pos_ == "DET" or word.pos_ == "CCONJ" or word.pos_ == "ADP" or next_word.pos_ == "DET" or next_word.pos_ == "CCONJ" or next_word.pos_ == "ADP" :
                ...
                outlist.append(word.text)
                with open(("api/v2/textfiles/verbs") , "r", encoding="utf-8") as f:
                    vocab = nlp(f.read())
                    for token in vocab:
                        #try catch to handle the possibility that a comparison between to words (the theme word and the current word from the vocab) has never occured before
                        try:
                        ...
                        if token.i == (len(vocab)-1):
                            ...
                            verb_to_append = token
                    f.close()
    with open("api/v2/output/output.txt", mode='a') as f:
        f.write("\n")
        f.write("\n")
...
the_list = create_output_list_v2(doc,theme, pw, slw, flw)
