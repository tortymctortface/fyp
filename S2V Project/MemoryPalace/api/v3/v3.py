...
def create_output_list_v3(doc, in_start_word,pw,slw,flw, pwsw):
    ...
    with open("api/v3/output/scores.csv", mode='w+') as f:
        f.close()
    ...
    for name in doc:
        ...
        if (name.text.isalpha()) and (not(name.pos_ == "SPACE")):
            with open("api/v3/vocab/nouns.txt","r", encoding="utf-8") as f:
                ...
                for possible_trigger_word in vocab_list:
                    if (possible_trigger_word.pos_ == "SPACE") or (possible_trigger_word.text=='\n') or (not(possible_trigger_word.text.isalpha())) or (name.text == possible_trigger_word.text):
                        ...
                        #print(possible_trigger_word.text)
                        #print (result)
                        #print(similarity_score)
                        #print(phonetic_score)
                        #print(first_letter_score)
                        #print(secound_letter_score)
                ...
                f.close()
    with open("api/v3/output/output.txt", mode='w+') as f:
        f.write("Output List:")
        f.write("\n")
        f.write(str(result))
        f.close()
...
def create_output_csv(original,w1,w2,w3,l1,l2,l3,previous_word,pw,slw,flw,pwsw):
    ...
    x = 1
    with open("api/v3/output/scores.csv", mode='a') as csv_file:
    ...
    for word in topthreelist:
        if word.is_alpha:
            ...
            with open("api/v3/output/scores.csv", mode='a') as csv_file:
                ...
                csv_file.close()
            with open("api/v3/output/scores.csv", mode='a') as csv_file:
                fieldnames = ['Chosen at random from the list of words with equal total scores:']
                writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                writer.writeheader()
                writer.writerow({'Chosen at random from the list of words with equal total scores:':listoflists[x-1]}  )
                csv_file.close()
            x += 1
        csv_file.close()
